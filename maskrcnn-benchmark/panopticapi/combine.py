...
def combine_to_panoptic(proc_id, img_ids, img_id2img, inst_by_image,
    ...
    for idx, img_id in enumerate(img_ids):
        ...
        for ann in inst_by_image[img_id]:
            ...
            panoptic_ann['category_id'] = int(ann['category_id'])
            pan_segm_id[mask] = int(segment_id)
        ...
        for ann in sem_by_image[img_id]:
            ...
            panoptic_ann['id'] = int(segment_id)
            panoptic_ann['category_id'] = int(ann['category_id'])
            pan_segm_id[mask_left] = int(segment_id)
            segments_info.append(panoptic_ann)
...
def pancmp(a, b):
    if a['score'] > 0.725 and b['score'] > 0.725:
...
def combine_predictions(semseg_json_file, instseg_json_file, images_json_file,
    ...
    for img_id in inst_by_image.keys():
        inst_by_image[img_id] = sorted(inst_by_image[img_id], key=cmp_to_key(pancmp))
    ...
    imgs_ids_all = np.array(list(img_id2img.keys())) ##
