...
def save_checkpoint(state, iters, tag=''):
    if not os.path.exists("./supernet_models"):
        os.makedirs("./supernet_models")
    filename = os.path.join(
        "./supernet_models/{}checkpoint-{:06}.pth.tar".format(tag, iters))
    torch.save(state, filename)
    latestfilename = os.path.join(
        "./supernet_models/{}checkpoint-latest.pth.tar".format(tag))
...
def get_lastest_model():
    ...
    model_list = os.listdir('./supernet_models/')
    if model_list == []:
        return None, 0
    model_list.sort()
    lastest_model = model_list[-1]
    iters = re.findall(r'\d+', lastest_model)
    return './supernet_models/' + lastest_model, int(iters[0])
